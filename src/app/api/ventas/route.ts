...
export async function GET(request: NextRequest) {
  try {
    ...
    let ventasQ: any = await ctx.supabase
      .from("ventas")
      .select(
        "id, empresa_id, numero_control, moneda, tipo_cambio, subtotal, monto_iva, total, tipo_venta, plazo_dias, metodo_pago, fecha, genera_nota_remision, nota_remision_numero, anulada, anulada_at, anulada_motivo, cliente_id"
      )
    ...
    const ventas: Venta[] = ventasRows.map((r) => {
      ...
      return {
        ...
        fecha: r.fecha,
        anulada: (r as unknown as { anulada?: boolean }).anulada === true,
