...
export async function POST(request: NextRequest) {
  try {
    ...
    const { ventaId, numeroControl, fechaIso, notaRemisionNumero, facturaId, numeroFactura, facturaError } = await createVentaTransaccionalPg({
      schema,
    ...
    try {
      ...
      if (pagosArr && pagosArr.length > 0) {
        const VALID_METODOS = new Set(["efectivo", "transferencia", "tarjeta", "qr", "billetera", "otro"] as const);
        for (const p of pagosArr) {
          const raw = typeof p.metodo_pago === "string" ? p.metodo_pago : "efectivo";
          const m = (VALID_METODOS.has(raw as typeof VALID_METODOS extends Set<infer U> ? U : never)
            ? raw
        ...
        // Si hubo más de un método distinto, marcar la venta como MIXTO
        ...
        if (metodosUnicos.size > 1) {
          try {
            ...
            console.warn("[ventas/create] no se pudo marcar metodo_pago='mixto':", e instanceof Error ? e.message : e);
          }
        }
      } else {
    ...
    return NextResponse.json(successResponse({ venta, nota_remision_numero: notaRemisionNumero, factura_id: facturaId, numero_factura: numeroFactura, factura_error: facturaError }));
  } catch (err) {
