...
export default function HistorialNRPage() {
  ...
  const [filtroEstado, setFiltroEstado] = useState<"" | DemoNotaRemision["estado"]>("");
  const [filtroOrigen, setFiltroOrigen] = useState("");
  const [filtroDestino, setFiltroDestino] = useState("");
  const [busqueda, setBusqueda] = useState("");

  useEffect(() => { setNrs(getNRs()); }, []);

  const filtradas = useMemo(() => {
    return nrs.filter((n) => {
      if (filtroEstado && n.estado !== filtroEstado) return false;
      if (filtroOrigen && n.origen !== filtroOrigen) return false;
      if (filtroDestino && n.destino !== filtroDestino) return false;
      if (busqueda.trim()) {
        const q = busqueda.trim().toLowerCase();
        if (!n.numero.toLowerCase().includes(q) && !n.emisor.toLowerCase().includes(q)) return false;
      }
      return true;
    });
  }, [nrs, filtroEstado, filtroOrigen, filtroDestino, busqueda]);
                  ...
                  <tr key={nr.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60">
                    ...
                    <td className="px-5 py-3 text-right">
                      <div className="inline-flex gap-1.5">
                        <a href={`/notas-remision/${nr.id}/documento`} target="_blank" rel="noopener" className="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50">📄</a>
                        {nr.estado === "pendiente" && (
