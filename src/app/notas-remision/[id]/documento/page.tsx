...
import { PRODUCTOS_DEMO, getNR, nombreUbicacion, type DemoNotaRemision } from "@/lib/demo-multideposito/store";
...
export default function DocumentoNRPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!cargado) return <div className="p-8 text-sm text-slate-500">Cargando…</div>;
  if (!nr) return (
    <div className="p-8 text-sm text-slate-500">
      NR no encontrada. <a href="/notas-remision" className="text-emerald-700 hover:underline">Volver al historial</a>
    </div>
  );
      ...
      <div className="bg-white border border-slate-300 rounded-lg p-8 print:border-0 print:rounded-none print:p-4">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4 border-t border-slate-100 pt-4">
          <div className="rounded-md bg-slate-50 border border-slate-200 p-3">
            ...
            <p className="mt-1 text-sm font-bold text-slate-800">{nombreUbicacion(nr.origen)}</p>
          </div>
          <div className="rounded-md bg-emerald-50 border border-emerald-200 p-3">
            <p className="text-[10px] uppercase tracking-wider text-emerald-800 font-semibold">Depósito Destino</p>
            <p className="mt-1 text-sm font-bold text-emerald-900">{nombreUbicacion(nr.destino)}</p>
          </div>
        </div>

        {(nr.transporte.transportista || nr.transporte.conductor || nr.transporte.chapa) && (
          <div className="mb-4 rounded-md border border-slate-200 p-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              <Info label="RUC transportista" value={nr.transporte.ruc_transportista ?? "—"} />
              <Info label="Conductor" value={nr.transporte.conductor ?? "—"} />
              <Info label="CI conductor" value={nr.transporte.ci_conductor ?? "—"} />
              <Info label="Chapa" value={nr.transporte.chapa ?? "—"} />
              <Info label="Inicio → Fin traslado" value={`${fmtFecha(nr.transporte.fecha_inicio_traslado)} → ${fmtFecha(nr.transporte.fecha_fin_traslado)}`} />
            </div>
          </div>
        )}
        ...
        <table className="w-full text-sm border border-slate-200 rounded overflow-hidden">
          <thead>
            <tr className="bg-slate-100 text-left text-xs uppercase tracking-wider text-slate-600">
              ...
              <th className="px-3 py-2 border-b border-slate-200 text-right">Unidad</th>
            </tr>
          </thead>
          <tbody>
            {nr.items.map((it) => {
              ...
              return (
                <tr key={it.producto_id} className="border-b border-slate-100 last:border-0">
                  ...
                  <td className="px-3 py-2 font-semibold text-slate-800">{p?.nombre ?? it.producto_id}</td>
                  <td className="px-3 py-2 text-right tabular-nums text-slate-700">{fmt(it.cantidad)}</td>
                  <td className="px-3 py-2 text-right text-slate-600 text-xs">{p?.unidad ?? "—"}</td>
                </tr>
              );
            })}
